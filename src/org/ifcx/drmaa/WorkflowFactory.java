...
public abstract class WorkflowFactory {
    ...
    public static WorkflowFactory getFactory() {
        synchronized (WorkflowFactory.class) {
            if (thisFactory == null) {
                ...
                thisFactory =
                        (WorkflowFactory)AccessController.doPrivileged(action);
            }
        }
    ...
    private static WorkflowFactory newFactory() throws ConfigurationError {
        ...
        try {
            String systemProp = System.getProperty(WORKFLOW_FACTORY_PROPERTY);

            if (systemProp != null) {
                return (WorkflowFactory)newInstance(systemProp, classLoader);
            }
        ...
        try {
            ...
            if (f.exists()) {
                ...
                String className = props.getProperty(WORKFLOW_FACTORY_PROPERTY);

                return (WorkflowFactory)newInstance(className, classLoader);
            }
        } catch (SecurityException se ) {
            // If we get a security exception, treat it as failure and try the
            // next method
        ...
        String serviceId = "META-INF/services/" + WORKFLOW_FACTORY_PROPERTY;
        ...
        throw new ConfigurationError("Provider for " + WORKFLOW_FACTORY_PROPERTY +
                " cannot be found", e);
    }
    ...
    private static ClassLoader findClassLoader() {
        ...
        try {
            ...
            classLoader = WorkflowFactory.class.getClassLoader();
        } catch (Exception ex) {
